...
def _parse_data_section(lines, previous_sections):
    ...
    data = _np.array(" ".join(lines).split(), dtype=float)
    data[data == nullvalue] = _np.nan
...
def read(lasfile):
    ...
    if isinstance(lasfile, _io.IOBase):
        lasfile.seek(0)
        close_file = False
    else:
...
def _compose_data_section(data, format, previous_sections):
    ...
    for i in range(data.shape[1]):
        nanfreeline = data[:, i]
        nanfreeline[_np.isnan(nanfreeline)] = nullvalue
